...
function generateReport() {
    ...
    if (report_period == "hourly") {
        ...
        url_string = "/getHourlyVisitFrequencyReport" + `?year=${split_string[0]}&month=${split_string[1]}&day=${split_string[2]}`;
        x_axis_label = `Hour`;
    }
    else if (report_period == "daily") {
        var split_string = report_time.split("-");
        url_string = "/getDailyVisitFrequencyReport" + `?year=${split_string[0]}&week=${split_string[1].substr(1)}`;
        x_axis_label = `Day`;
    }
    else if (report_period == "weekly") {
        var split_string = report_time.split("-");
        url_string = "/getWeeklyVisitFrequencyReport" + `?year=${split_string[0]}&month=${split_string[1]}`;
        x_axis_label = `Week`;
    }
    else {
        url_string = "/getMonthlyVisitFrequencyReport" + `?year=${report_time}`;
        x_axis_label = `Month`;
    }
    ...
    $.ajax({
        ...
        "complete": (data) => {
            clean_graph();
            draw_graph(data.responseJSON, x_axis_label, "Visit Frequency");
        }
    });
}